...
//升降序
$sql4 ="SELECT * FROM goodslist WHERE category='$category' ORDER BY gNowPrice $desc LIMIT $index,$num";
$res4 = $conn->query($sql4);
$content4 = $res4->fetch_all(MYSQLI_ASSOC);
//价格区间
$min = isset($_POST['min']) ? $_POST['min'] : '';
$max = isset($_POST['max']) ? $_POST['max'] : '';
$sql5 ="SELECT * FROM goodslist WHERE category='$category' AND gNowPrice BETWEEN '$min' AND '$max' LIMIT $index,$num";
$res5 = $conn->query($sql5);
$content5 = $res5->fetch_all(MYSQLI_ASSOC);
//模糊查询
$keyword = isset($_POST['keyword']) ? $_POST['keyword'] : '';
$sql6 ="SELECT * FROM goodslist WHERE category='$category' AND gName LIKE '%$keyword%' LIMIT $index,$num";
...
//价格区间总条数
$sql8 ="SELECT * FROM goodslist WHERE category='$category' AND gNowPrice BETWEEN '$min' AND '$max'";
$res8 = $conn->query($sql8);
//模糊查找总条数 
$sql9 ="SELECT * FROM goodslist WHERE category='$category' AND gName LIKE '%$keyword%'";
...
$data = array(
   ...
   'chose'=> $content5,//价格区间
    ...
    'pages2' => $res8->num_rows,//价格区间总条数
    'pages3' => $res9->num_rows,//模糊查找总条数 
);
